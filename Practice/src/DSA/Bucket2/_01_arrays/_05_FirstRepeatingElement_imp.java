...
public class _05_FirstRepeatingElement_imp {
	...
	public static boolean binarySearch(int[] temp, int ele) {
		...
		while (l <= r) {
			...
			if (temp[mid] == ele) {
				...
				if ((mid - 1 > 0 && temp[mid - 1] == ele) || (mid + 1 < temp.length && temp[mid + 1] == ele))
					return true;
	...
	public static int firstRepeating_Approach1(int[] arr) {
		...
		int[] temp = arr.clone(); // O(n)
		Arrays.sort(temp); // O(nlogn)

		for (int i = 0; i < arr.length; i++) {// O(n)
			if (binarySearch(temp, arr[i]))// O(logn)
				return arr[i];
		}
