...
public class _09_AVL_Delete {
	...
	public Node avlInsert(Node root, int key) {
		...
		// after inserting node, check for bf of all its parent nodes till root
	...
	public Node avlDelete(Node root, int key) {
		...
		// Found key to be deleted
			...
			// Below condition when node to be deleted is with 2 children
				...
				root.key = findMinRightSubTreeValue(root.right).key;
				root.right = avlDelete(root.right, root.key);
			}
		}

		// after deleting node, check for bf of all its parent nodes till root
		int bf = getBalanceFactor(root); // (height of Left Sub Tree) - (height of Right Sub Tree)

		// below checks, can i make use of same conditions as insert?
	...
	public static void main(String[] args) {
		_09_AVL_Delete tree = new _09_AVL_Delete();
